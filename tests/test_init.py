"""Tests for La Poste setup and unload."""
...
from custom_components.laposte.api import LaPosteApiError
...
async def test_setup_and_unload(hass):
    ...
    with patch(
        "custom_components.laposte.api.LaPosteApiClient.async_get_parcel",
    ...
    incoming = hass.states.get("sensor.la_poste_incoming_parcels")
...
async def test_setup_retries_when_first_refresh_fails(hass):
    ...
    with patch(
        "custom_components.laposte.api.LaPosteApiClient.async_get_parcel",
        new=AsyncMock(side_effect=LaPosteApiError("La Poste unreachable")),
    ):
...
async def test_per_parcel_sensor_spawn_and_remove(hass):
    ...
    with patch("custom_components.laposte.api.LaPosteApiClient.async_get_parcel", new=mock):
        assert await hass.config_entries.async_setup(entry.entry_id)
        await hass.async_block_till_done()
