"""Coordinator for the La Poste parcel tracker integration.
...
from .api import LaPosteApiClient, LaPosteApiError
from .const import (
...
class LaPosteCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        for code, result in zip(codes_to_fetch, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (LaPosteApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                if isinstance(result, LaPosteApiError) and result.status_code == 429:
                    saw_429 = True
                    if result.retry_after is not None:
                        retry_afters.append(result.retry_after)
                _LOGGER.warning("La Poste fetch failed for %s: %s", code, result)
        ...
        if saw_429:
            ...
            raise UpdateFailed(
                "La Poste rate-limited (429)", retry_after=retry_after
            )
        self._consecutive_429 = 0

        if codes_to_fetch and errors == len(codes_to_fetch) and not raws_by_code:
            raise UpdateFailed("La Poste unreachable for all tracked parcels")
