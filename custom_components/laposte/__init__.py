"""La Poste parcel tracker custom component for Home Assistant."""
...
from .api import LaPosteApiClient
...
@dataclass
class LaPosteData:
    """Runtime data attached to the La Poste config entry."""

    client: LaPosteApiClient
    coordinator: LaPosteCoordinator


type LaPosteConfigEntry = ConfigEntry[LaPosteData]


async def async_setup_entry(hass: HomeAssistant, entry: LaPosteConfigEntry) -> bool:
    """Set up La Poste from a config entry."""
    # No auth: La Poste tracking is public, so the HA-managed session is fine.
    client = LaPosteApiClient(async_get_clientsession(hass))
    ...
    entry.runtime_data = LaPosteData(client=client, coordinator=coordinator)
...
async def _async_options_updated(
    hass: HomeAssistant, entry: LaPosteConfigEntry
) -> None:
    """Apply changed options by refreshing the coordinator."""
    await entry.runtime_data.coordinator.async_request_refresh()


async def async_unload_entry(hass: HomeAssistant, entry: LaPosteConfigEntry) -> bool:
    """Unload the La Poste config entry."""
