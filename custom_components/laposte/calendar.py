"""Calendar platform for the La Poste parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the La Poste deliveries calendar from a config entry."""
    async_add_entities([LaPosteDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class LaPosteDeliveriesCalendar(CoordinatorEntity[LaPosteCoordinator], CalendarEntity):
    """A read-only calendar of expected La Poste deliveries.
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "La Poste parcel")
            description_parts = [
