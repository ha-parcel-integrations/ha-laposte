"""Sensor platform for the La Poste parcel tracker integration."""
...
from . import LaPosteConfigEntry
from .const import DOMAIN, ParcelStatus
from .coordinator import LaPosteCoordinator
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: LaPosteConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up La Poste sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        LaPosteIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
        LaPosteAwaitingPickupSensor(coordinator, entry),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            LaPosteParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(LaPosteNextDeliverySensor(coordinator, entry))
    entities.append(LaPosteDeliveredParcelsSensor(coordinator, entry))
    entities.append(LaPosteLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class LaPosteAwaitingPickupSensor(
    CoordinatorEntity[LaPosteCoordinator], SensorEntity
):
    ...
    def __init__(self, coordinator: LaPosteCoordinator, entry: ConfigEntry) -> None:
...
class LaPosteIncomingParcelsSensor(
    CoordinatorEntity[LaPosteCoordinator], SensorEntity
):
    ...
    def __init__(
        self,
        coordinator: LaPosteCoordinator,
        entry: ConfigEntry,
        async_add_entities: AddEntitiesCallback,
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                LaPosteParcelSensor(self.coordinator, self._entry, barcode)
...
class LaPosteParcelSensor(CoordinatorEntity[LaPosteCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked La Poste parcel."""
    ...
    def __init__(
        self, coordinator: LaPosteCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class LaPosteNextDeliverySensor(
    CoordinatorEntity[LaPosteCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: LaPosteCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class LaPosteDeliveredParcelsSensor(
    CoordinatorEntity[LaPosteCoordinator], SensorEntity
):
    """Recently delivered tracked La Poste parcels."""
...
class LaPosteLastUpdateSensor(
    CoordinatorEntity[LaPosteCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when La Poste was last polled successfully."""
