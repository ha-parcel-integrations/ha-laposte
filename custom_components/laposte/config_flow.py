"""Config flow for the La Poste parcel tracker integration."""
...
class LaPosteConfigFlow(ConfigFlow, domain=DOMAIN):
    """Handle the UI-driven configuration flow for the La Poste integration."""
    ...
    ) -> LaPosteOptionsFlowHandler:
        """Return the options flow handler."""
        return LaPosteOptionsFlowHandler()

    async def async_step_user(
        self, user_input: dict[str, Any] | None = None
    ) -> ConfigFlowResult:
        """Create the La Poste hub — single instance, no input needed.
        ...
        the ``laposte.track_parcel`` service or a dashboard button.
...
class LaPosteOptionsFlowHandler(OptionsFlow):
    """Manage tracked parcels and settings behind a two-step menu.

    ``init`` shows a menu (``Parcels`` / ``Settings``); ``async_step_parcels``
    edits the tracked-code list, ``async_step_settings`` is a flat form for
    delivered-filter and history options. Changes apply live via HA's
